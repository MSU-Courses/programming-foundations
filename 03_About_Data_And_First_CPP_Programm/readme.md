...
- [Данные в программе. Первая программа на C++](#данные-в-программе-первая-программа-на-c)
  ...
  - [Что такое данные и информация](#что-такое-данные-и-информация)
    ...
    - [Данные во время и после работы программы](#данные-во-время-и-после-работы-программы)
  ...
  - [Переменная и литерал](#переменная-и-литерал)
    ...
    - [Где в памяти лежит переменная](#где-в-памяти-лежит-переменная)
...
7. Чем отличаются записи `7`, `7.0`, `'7'` и `"7"`? К каким видам литералов они относятся?
...
В отличие от данных, **информация** - это осмысленные и полезные для человека сведения, которые можно извлечь из данных.
...
### Данные во время и после работы программы

Во время работы значения обычно находятся в оперативной памяти, выделенной процессу. После завершения программы эта память освобождается, поэтому находившиеся только в ней значения теряются.
...
Теперь разберём, как последовательность битов превращается в число в программе.
...
**Кодировка (encoding)** - это правило представления символов с помощью чисел и байтов.
...
![Рисунок 3.2. Символ превращается в номер по кодировке, а номер хранится в памяти как набор битов](../_images/03/fig-char-code.svg)

_Рисунок 3.2. Как символ оказывается в памяти_

![Рисунок 3.3. Таблица кодов ASCII: печатные символы и соответствующие им числа](../_images/03/ascii-table.webp)

_Рисунок 3.3. Таблица кодов ASCII_

Современный стандарт **Unicode** охватывает символы практически всех письменностей мира, включая кириллицу и иероглифы[^6]. Unicode назначает символам кодовые точки, а кодировки UTF-8 и UTF-16 определяют, как представить их последовательностями байтов.

> [!IMPORTANT]
>
> Обратите особое внимание на цифры. В ASCII и совместимых с ней кодировках символу `'5'` соответствует код 53. _Символ `'5'` и число `5` - это разные значения_, и на этом различии студенты ошибаются чаще всего.
...
> [!WARNING]
>
> Многие дробные числа хранятся лишь приближённо. Например, десятичные дроби `0.1` и `0.2` нельзя точно представить конечной двоичной дробью.
...
Именно в этом и состоит работа имени: оно скрывает номера байтов, с которыми человеку работать неудобно. Программист пишет `health`, а компилятор сам находит нужное место в памяти.
...
### Где в памяти лежит переменная

Осталось понять, что означает "место в памяти" из определения переменной.

Память это длинная последовательность байтов, и каждый байт имеет свой номер. Номер байта называют **адресом**.

Когда программа создаёт переменную, ей достаётся несколько подряд идущих байтов. Сколько именно, зависит от вида данных: для одного символа хватает одного байта, для целого числа обычно отводят четыре.

![Рисунок 3.4. Последовательность пронумерованных байтов: переменная health занимает четыре байта подряд, переменная symbol один байт](../_images/03/fig-memory-layout.svg)

_Рисунок 3.4. Как переменные размещены в памяти_

В байтах, как и раньше, лежат только биты. Число 75 не хранится десятичной записью: четыре байта переменной `health` содержат `00000000`, `00000000`, `00000000` и `01001011`, и лишь все вместе они означают 75.

Номера байтов на схеме взяты для примера. Настоящие номера выбирает компилятор, и знать их не нужно: именно для этого у переменной есть имя. Вы пишете `health`, а компилятор сам помнит, где эта переменная лежит и сколько места занимает.

> [!NOTE]
>
> В той же памяти находятся и сами инструкции программы, о чём шла речь в первой главе. Для процессора инструкции и данные это одинаковые байты.
...
2. _Типы контролируются строго._ В первых программах мы будем указывать тип каждой переменной явно, это помогает компилятору проверять правильность операций с данными.
3. _Он позволяет работать близко к машине._ Программист может выбирать подходящие типы и управлять памятью гораздо точнее, чем во многих других языках. Поэтому разговор о байтах здесь особенно важен. C++ широко применяется в игровой индустрии: на нём написан Unreal Engine и значительная часть других движков.
...
>
> - `main.cpp` можно открыть и прочитать как текст, но процессор не исполняет его напрямую;
> - `main.exe` - это исполняемый файл, который можно запустить. Его содержимое уже предназначено для компьютера, а не для чтения человеком.
...
Большинство простых инструкций в C++ заканчивается **точкой с запятой**. Это не украшение: она помогает компилятору определить границу инструкции.
...
- Имя переменной состоит из латинских букв, цифр и знака подчёркивания;
...
int player_health = 25; // слова можно разделять знаком подчёркивания
...
Инициализировать переменную рекомендуется всегда, и вот почему:

```cpp
int health;              // начальное значение не задано

std::cout << health;     // ошибка: чтение неинициализированной переменной
```

Здесь переменная объявлена, но начальное значение ей не задано. Такое чтение приводит к **неопределённому поведению**: стандарт C++ не обещает ни конкретного числа, ни даже нормального продолжения работы программы. Компилятор может предупредить об ошибке, но не обязан отклонять программу.
...
| Логический тип         | `bool`    | `true`, `false`       | Жив ли персонаж, есть ли ключ, пауза | `false` = 0, `true` = 1                          |
...
**`bool`** принимает ровно два значения, `true` и `false`, и применяется для признаков состояния. Это литералы, а не текст, кавычки им не нужны.
...
В обоих инициализаторах записан символьный литерал `'A'`. При присваивании переменной `code` он преобразуется в целое число. В распространённых ASCII-совместимых кодировках это число 65. Результаты вывода различаются из-за типов: `char` выводится как символ, а `int` - как число. Размеры и внутреннее представление этих переменных при этом не обязаны совпадать.
...
При обычном формате вывода `x` может выглядеть как `0.3`, хотя внутри хранится близкое, но не равное ему значение. Поэтому сравнение `x == 0.3` обычно оказывается ложным.

Вывод: _результаты вычислений с числами с плавающей точкой обычно не сравнивают на точное равенство_. Как сравнивать правильно, разберём при изучении условий.

## Ввод и вывод в C++

Для ввода и вывода (по умолчанию - через консоль) в C++ используются объекты `std::cin` и `std::cout`. Они становятся доступны после строки `#include <iostream>`.

### Вывод на экран

Рассмотрим пример вывода на экран:

```cpp
std::cout << "Hello!" << '\n';
```

- `std::cout` - стандартный поток вывода, который обычно связан с консолью;
- `<<` - показывает направление: значение отправляется в поток вывода;
- `"Hello!"` - текст, который выводится как есть;
...
2. Значения попадают в программу тремя путями: записываются в тексте программы, получаются извне или вычисляются из других данных. Для использования после завершения программы данные сохраняют в файле, базе данных или другом постоянном хранилище.
...
10. Деление целых даёт целое, дробная часть при записи в `int` теряется, а результаты вычислений с числами с плавающей точкой обычно не сравнивают на точное равенство.
