...
- [Выражения и операции](#выражения-и-операции)
  ...
  - [Приоритет операций](#приоритет-операций-1)
  - [Когда в выражении встречаются разные типы](#когда-в-выражении-встречаются-разные-типы)
...
Чтобы получить дробный результат, хотя бы один операнд должен быть дробным:
...
Операции одного приоритета выполняются слева направо. Полная таблица приоритетов в C++ намного длиннее, в ней несколько десятков строк[^2], но для наших задач хватает того, что приведено здесь.
...
Теперь зная все операции, рассмотрим полный порядок их приоритетов:

| Приоритет    | Операции          | Пояснение               |
| ------------ | ----------------- | ----------------------- |
| 1, выше всех | `!`               | отрицание               |
| 2            | `*` `/` `%`       | умножение и деление     |
| 3            | `+` `-`           | сложение и вычитание    |
| 4            | `<` `<=` `>` `>=` | сравнения по величине   |
...
## Когда в выражении встречаются разные типы
...
10. Тип переменной слева не влияет на вычисление справа, поэтому для дробного результата применяют явное преобразование `static_cast<double>`.
